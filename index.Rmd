...
***=left
**Sera Sample**
```{r sera_sample, results='asis', echo=FALSE, comment=FALSE, warning=FALSE, cache=TRUE,message=FALSE}
...
```{r nvd3plot1, echo=FALSE, results ='asis'}
require(rCharts)
lhd <- read.csv("options.csv")
...
```
```{r livelihoods1, results = 'asis', comment = NA, message = F, echo = F}
#d1$show('inline')
...
```{r nvd3plot2, echo=F,results='asis'}
require(rCharts)
...
#n2$print('chart2')
```
```{r wildlife1, results = 'asis', comment = NA, message = F, echo = F}
#d1$show('inline')
...
library(plyr)
library(XLConnect)
gov <- loadWorkbook("gov_scores.xlsx")
...
```{r nvd3plot3, echo=FALSE, results ='asis'}
require(rCharts)
...
```
```{r security1, results = 'asis', comment = NA, message = F, echo = F}
#d1$show('inline')
